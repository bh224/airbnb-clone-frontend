...
export const googleLogin = (token: string) => instance.post(`/users/google`, { token }, {
    headers: {
        "X-CSRFToken": Cookie.get("csrftoken") || ""
    },
}).then(response => response.status)
...
export const usenameLogIn = ({ username, password }: IUsernameLoginVariables) =>
    instance.post(
        `/users/log-in`,
        { username, password },
        {
...
export const getCategories = ({ queryKey }:QueryFunctionContext) => {
...
export const checkBooking = ({ queryKey }: QueryFunctionContext<CheckBookingQueryKey>)=> {
    ...
    if (dates) {
        ...
        return instance.get(`rooms/${roomPk}/bookings/check?check_in=${checkIn}&check_out=${checkOut}`).then((response)=>response.data)
    }
}